...
export async function GET(request: NextRequest) {
  try {
    // const { ClassCustomer } = await request.json();

    // if (!ClassCustomer) {
    //   const res = NextResponse.json({ error: "Invalid Class Customer" }, { status: 400 });
    //   res.headers.set("Cache-Control", "no-store");
    //   return res;
    // }

    const statusRes = await fetch(`https://vipvsq.vsquareclinic.com/api/service/crm`, {
      method: 'GET',
      cache: 'no-store',
      headers: {
    ...
    const res = NextResponse.json({ error: "Invalid Class Customer" }, { status: 401 });
    res.headers.set("Cache-Control", "no-store");
    return res;
  }
}
